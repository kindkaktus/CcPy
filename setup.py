...
def usage():
    print >>sys.stderr,"usage: python setup.py [install]"
    print >>sys.stderr,"       python setup.py uninstall"

def install():
    print "Installing CcPy..."
    print "  Checking Python version..."
    if  sys.version_info[0] < 2 or ( sys.version_info[0] == 2 and sys.version_info[1] < 5 ):
        print >>sys.stderr,"*** Error: Python 2.5 or higher is required."
        return -1
    ...
    try:
        ...
        print >>sys.stderr,"*** Error: python-expat module is required."
        return -1
...
def uninstall():
    ...
    try:
        ...
        print >>sys.stderr,"*** Failed to stop ccpyd. %s" % str(e)
        return -1
...
def _removeFileNoThrow(aFileName):
    ...
    try:
        ...
        print >>sys.stderr,"Warning. Cannot remove '%s'. Error: %s" % (aFileName, str(e))
        return 1
