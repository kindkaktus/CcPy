...
class CcPyState:
    ...
    def getPrjState(self, aName):
        if not os.path.exists(self._fileName):
            Logger.debug("File '%s' does not exist, defaulting project state to %s" % (self._fileName, PrjStates.UNKNOWN))
        ...
        if (len(myRequestedProjects) == 0):
            Logger.debug("'%s' does not contain project '%s', defaulting project state to %s" % (self._fileName, aName, PrjStates.UNKNOWN))
            return PrjStates.UNKNOWN
    ...
    def setPrjState(self, aName, aVal):
        ...
        if os.path.exists(self._fileName):
            ...
            if (len(myProjects2Change) == 0):
                Logger.debug("'%s' does not contain project '%s', adding project with state %s" % (self._fileName, aName, aVal))
                ...
                Logger.debug("'%s' contains project '%s', setting project state to %s" % (self._fileName, aName, aVal))
