...
class ExecTask(task.Task):
    ...
    def execute(self):
        try:
            ...
            # grab stdout/stderr below without blocking
            Logger.debug(
            ...
            if myProcess.returncode == 0:
                return {
                    "statusFlag": True,
                    "statusDescr": "The execution of '%s %s' in %s completed successfully." %
                    (self._executable,
                     self._args,
                     self._workingDir),
                    "stdout": myStdoutConsumer.out,
            ...
            if self._warningExitCode is not None and myProcess.returncode == self._warningExitCode:
                return {
                    "statusFlag": True,
                    "warning": True,
                    "statusDescr": "The execution of '%s %s' in %s completed with warning and return code %d." %
                    (self._executable,
                     self._args,
                     self._workingDir,
            ...
            return {
                "statusFlag": False,
                "statusDescr": "The execution of '%s %s' in %s finished with return code %d." %
                (self._executable,
                 self._args,
                 self._workingDir,
                 myProcess.returncode),
            ...
            return {
                "statusFlag": False,
                "statusDescr": "Failed to execute '%s %s' in %s. Error: %s" %
                (self._executable,
                 self._args,
                 self._workingDir,
                 str(e))}
